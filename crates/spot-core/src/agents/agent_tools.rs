...
use super::{AgentExecutor, AgentManager, RetryHandler};
...
/// For example, the main spot agent might delegate code review
...
mod tests {
    ...
    #[tokio::test]
    async fn test_list_agents_tool_includes_spot() {
        let tool = ListAgentsTool;
        let ctx = RunContext::minimal("test");
        ...
        let spot = agents
            .iter()
            .find(|a| a.get("name").unwrap().as_str().unwrap() == "spot");
        assert!(spot.is_some(), "should include spot agent");
    }
