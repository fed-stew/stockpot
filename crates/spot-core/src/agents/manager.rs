...
impl AgentManager {
    ...
    pub fn new() -> Self {
        let mut manager = Self {
            agents: HashMap::new(),
            current_agent: Arc::new(RwLock::new("spot".to_string())),
        };
    ...
    /// Register JSON-defined agents from ~/.spot/agents/.
...
mod tests {
    ...
    #[test]
    fn test_new_sets_default_agent_to_spot() {
        let manager = AgentManager::new();
        assert_eq!(manager.current_name(), "spot");
    }
    ...
    fn test_get_existing_agent() {
        ...
        let agent = manager.get("spot");
        assert!(agent.is_some());
        assert_eq!(agent.unwrap().name(), "spot");
    }
    ...
    fn test_current_returns_default_agent() {
        ...
        assert_eq!(current.unwrap().name(), "spot");
    }
    ...
    fn test_switch_to_same_agent() {
        ...
        let result = manager.switch("spot");
        assert!(result.is_ok());
    }
    ...
    fn test_agent_info_includes_visibility() {
        ...
        let spot = agents.iter().find(|a| a.name == "spot");
        assert!(spot.is_some());
        assert_eq!(spot.unwrap().visibility, AgentVisibility::Main);
    }
    ...
    fn test_concurrent_switch_operations() {
        ...
        for i in 0..10 {
            ...
            let agent = if i % 2 == 0 { "planning" } else { "spot" };
        ...
        assert!(name == "planning" || name == "spot");
    }
    ...
    fn test_overwrite_builtin_agent() {
        ...
        let original = manager.get("spot").unwrap().description().to_string();

        // Overwrite with mock
        manager.register(Box::new(
            MockAgent::new("spot").with_description("Overwritten"),
        ));

        // Should have new description
        let new = manager.get("spot").unwrap().description();
    ...
    fn test_overwrite_preserves_agent_count() {
        ...
        manager.register(Box::new(MockAgent::new("spot")));
    ...
    fn test_current_after_overwriting_current_agent() {
        ...
        manager.register(Box::new(
            MockAgent::new("spot").with_description("New Spot"),
        ));

        // current() should return the new version
        let current = manager.current().unwrap();
        assert_eq!(current.description(), "New Spot");
    }
