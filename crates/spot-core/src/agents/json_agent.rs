...
//!
//! Loads agent definitions from `~/.spot/agents/*.json`.
//!
//! ## JSON Schema
//!
//! ```json
...
pub fn agents_dir() -> PathBuf {
    dirs::home_dir()
        .map(|h| h.join(".spot").join("agents"))
        .unwrap_or_else(|| PathBuf::from(".spot/agents"))
}
...
mod tests {
    ...
    fn test_agents_dir_returns_path() {
        let dir = agents_dir();
        // Should end with .spot/agents
        assert!(dir.ends_with("agents"));
        let parent = dir.parent().unwrap();
        assert!(parent.ends_with(".spot"));
    }
