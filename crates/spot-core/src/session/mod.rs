...
//! ## Storage Format
//!
//! Sessions are stored in `~/.spot/sessions/` as:
...
//!
//! ```ignore
//! use spot_core::session::SessionManager;
//!
//! let manager = SessionManager::new();
//!
//! // Save a session
//! manager.save("my-project", &messages, "spot", "gpt-4o")?;
//!
//! // List sessions
...
mod tests {
    ...
    fn test_session_meta_new() {
        let meta = SessionMeta::new("test-session", "spot", "gpt-4o");

        assert_eq!(meta.name, "test-session");
        assert_eq!(meta.agent, "spot");
    ...
    fn test_session_data_new() {
        let data = SessionData::new("my-session", "spot", "gpt-4o");

        assert_eq!(data.meta.name, "my-session");
        assert_eq!(data.meta.agent, "spot");
    ...
    fn test_session_manager_default() {
        ...
        // Should use home dir
        assert!(manager.sessions_dir().to_string_lossy().contains("spot"));
    }
    ...
    fn test_session_manager_save_load() {
        ...
        // Save
        let meta = manager
            .save("test-session", &messages, "spot", "gpt-4o")
            .unwrap();
        assert_eq!(meta.name, "test-session");
        assert_eq!(meta.agent, "spot");
