...
pub fn prompt_api_key(db: &Database, provider: &ProviderInfo) -> Result<Option<String>> {
    ...
    if pool_key_count > 0 || db.has_api_key(env_var) {
        ...
        println!(
            "\x1b[32m✓ {} is already configured in spot ({})\x1b[0m",
            env_var, key_desc
        );
    ...
    // Also check environment variable
        ...
        if !existing.is_empty() {
            ...
            print!("\nUse existing key and save to spot? [Y/n]: ");
            io::stdout().flush()?;
            ...
            if input.is_empty() || input == "y" || input == "yes" {
                ...
                match db.save_pool_key(env_var, &existing, Some("From environment"), Some(0)) {
                    Ok(_) => {
                        println!("\x1b[32m✓ API key saved to spot database\x1b[0m");
                    }
                    Err(e) => {
    ...
    println!("  1. Enter API key now (saved securely in spot)");
    println!(
...
pub async fn run_add_model(db: &Database) -> Result<()> {
    println!("\n\x1b[1m👁️ Add Model Wizard\x1b[0m\n");
