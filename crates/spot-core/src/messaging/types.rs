...
mod tests {
    ...
    fn test_message_agent_started() {
        let msg = Message::agent_started("spot", "Spot");
        if let Message::Agent(agent_msg) = msg {
            assert_eq!(agent_msg.agent_name, "spot");
            assert_eq!(agent_msg.display_name, "Spot");
    ...
    fn test_message_agent_completed() {
        let msg = Message::agent_completed("spot", "Spot", "run-abc");
        if let Message::Agent(agent_msg) = msg {
            assert_eq!(agent_msg.agent_name, "spot");
            assert_eq!(agent_msg.display_name, "Spot");
    ...
    fn test_message_agent_error() {
        let msg = Message::agent_error("spot", "Spot", "failed to run");
        if let Message::Agent(agent_msg) = msg {
            assert_eq!(agent_msg.agent_name, "spot");
    ...
    fn test_agent_message_serde() {
        let msg = AgentMessage {
            agent_name: "spot".to_string(),
            display_name: "Spot".to_string(),
            event: AgentEvent::Started,
        };
        let json = serde_json::to_string(&msg).unwrap();
        let parsed: AgentMessage = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed.agent_name, "spot");
        assert_eq!(parsed.display_name, "Spot");
    }
