...
use spot_core::config::{PdfMode, Settings};
...
use spot_core::tools::{set_global_context, SpotToolRegistry, ToolContext};
...
pub struct ChatApp {
    ...
    is_generating: bool,
    /// Whether VDI mode is active (reduced frame rate for remote desktop)
    vdi_mode: bool,
    /// Whether a re-render is needed on the next animation tick
    needs_render: bool,
    ...
    add_model_providers: Vec<spot_core::models::catalog::ProviderInfo>,
    add_model_selected_provider: Option<String>,
    add_model_models: Vec<spot_core::models::catalog::ModelInfo>,
    ...
    key_pool_keys: Vec<spot_core::db::PoolKey>,
...
impl ChatApp {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        // Detect VDI mode (user setting overrides auto-detection)
        let vdi_mode = spot_core::config::is_vdi_mode_active(&settings);
        if vdi_mode {
            tracing::info!("VDI mode enabled - using reduced frame rate for better performance");
        }
    ...
    pub(super) fn do_mcp_import(&mut self, cx: &mut Context<Self>) {
        use spot_core::mcp::{McpConfig, McpServerEntry};
