...
impl ChatApp {
    pub(crate) fn render_settings_general(&self, cx: &Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            // =========================================================================
            // VDI Mode (Toggle with Description)
            // =========================================================================
            .child({
                let entity = entity.clone();
                let vdi_detected = spot_core::config::detect_vdi_environment();
                let vdi_active = self.vdi_mode;
                let label = if vdi_detected { "VDI Mode (auto-detected)" } else { "VDI Mode" };
                settings_row(
                    label,
                    Some("Reduces animation frame rate for Citrix/VDI/RDP environments.\nRequires restart to take effect."),
                    None,
                    false,
                    &theme,
                    toggle(
                        ...
                        move |_window, cx| {
                            entity.update(cx, |this, cx| {
                                let settings = Settings::new(&this.db);
                                // Toggle: if currently on, force off. If off, force on.
                                // This overrides auto-detection.
                                let new_mode = !this.vdi_mode;
                                settings.set_vdi_mode(Some(new_mode));
                                this.vdi_mode = new_mode;
                                cx.notify();
                            });
                        },
                    ),
