...
pub fn run_gui(config: AppConfig) -> Result<()> {
    ...
    if !config.skip_update_check {
        std::thread::spawn(|| {
            ...
            rt.block_on(async {
                if let Some(release) = spot_core::version_check::check_for_update().await {
                    tracing::info!(
                        "Update available: {} -> {}",
                        spot_core::version_check::CURRENT_VERSION,
                        release.version
                    );
                }
            });
            ...
            cx.open_window(
                WindowOptions {
                    ...
                    titlebar: Some(gpui::TitlebarOptions {
                        title: Some(SharedString::from("Spot")),
                        ..Default::default()
                    }),
                    ..Default::default()
                },
...
pub fn run_render_test() -> Result<()> {
    ...
    println!("╔═══════════════════════════════════════════════════════════════╗");
    println!("║           SPOT RENDER PERFORMANCE TEST                         ║");
    println!("╚═══════════════════════════════════════════════════════════════╝");
    println!();
