...
impl ChatApp {
    ...
    pub(super) fn fetch_providers(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this: WeakEntity<ChatApp>, cx: &mut AsyncApp| {
            match stockpot_core::models::catalog::fetch_providers().await {
                Ok(providers) => {
    ...
    pub(super) fn add_single_model(
        ...
        use stockpot_core::models::{CustomEndpoint, ModelConfig, ModelType};
    ...
    pub(super) fn start_oauth_flow(&mut self, provider: &'static str, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this: WeakEntity<ChatApp>, cx: &mut AsyncApp| {
            let result = match provider {
                "chatgpt" => stockpot_core::auth::run_chatgpt_auth(&db)
                    .await
                    .map_err(|e| e.to_string()),
                "claude-code" => stockpot_core::auth::run_claude_code_auth(&db)
                    .await
                    .map_err(|e| e.to_string()),
                "google" => stockpot_core::auth::run_google_auth(&db)
                    .await
                    .map_err(|e| e.to_string()),
