//! Main UI rendering
//!
//! Uses spot-style layout: Header → ActivityFeed → Input → StatusBar
...
pub fn render(frame: &mut Frame, app: &mut TuiApp) {
    ...
    // Use spot-style layout with dynamic input height
...
fn render_header(frame: &mut Frame, app: &mut TuiApp, area: Rect) {
    ...
    // Combined agent/model dropdown trigger (after "spot │ ")
    // Clicking anywhere on "Agent • model ▾" opens the agent dropdown
    app.hit_registry.register(
        Rect::new(area.x + 11, area.y, agent_section_width, 1),
...
fn render_welcome(frame: &mut Frame, area: Rect) {
    let welcome = Paragraph::new(vec![
        Line::from(""),
        Line::from(""),
        Line::from(Span::styled("👁️", Style::default())),
        Line::from(""),
        ...
        Line::from(Span::styled(
            "Spot what matters. Control everything.",
...
fn render_agent_dropdown(frame: &mut Frame, app: &mut TuiApp, header_area: Rect) {
    ...
    // Position below the agent section in header (after "spot │ ")
