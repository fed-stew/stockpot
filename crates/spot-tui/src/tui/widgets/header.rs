//! Header bar widget
//!
//! Displays spot branding with combined agent/model selector and working folder.
//! Design: 👁️ spot │ Agent 🐶 • model ▾ │ 📁 folder ▾ │ F2 ⚙
//!
...
pub struct Header {
    /// Agent display name (e.g., "Control Agent 👁️")
...
impl Header {
    ...
    ///
    /// # Arguments
    /// * `agent_display` - The agent's display name (e.g., "Control Agent 👁️")
    ...
    pub fn folder_offset(&self) -> u16 {
        // " 👁️ " (4) + "spot" (4) + " │ " (3) + agent_section + " │ " (3)
        (4 + 4 + 3 + self.agent_section_width() as usize + 3) as u16
    }
